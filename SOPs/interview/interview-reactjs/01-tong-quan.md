# Tổng quan — Skill phỏng vấn React / Frontend Developer

> Khái niệm level, mục tiêu năng lực, trọng số, cấu trúc bộ tài liệu và luồng sử dụng dự kiến.  
> Lịch sử trao đổi yêu cầu: [../brainstoming.md](../brainstoming.md) · [../brainstoming-reactjs.md](../brainstoming-reactjs.md) · Danh sách file: [README.md](./README.md)
...
| Bước | Ai | Việc |
|------|-----|------|
| 1 | Ứng viên | Trả lời bộ câu hỏi & bài (theo rubric trong `interview-reactjs/`) |
...
Đánh giá ứng viên **React / Frontend** theo thang **9 level** (Junior 1–3, Mid 1–3, Senior 1–3), dựa trên:

| Nhóm | Nội dung |
|------|----------|
| **Cơ bản** | JSX/Virtual DOM, props/state, hooks cơ bản, effect, controlled input, key, event, styling, fetch |
| **Nâng cao** | Custom hooks, Context, memo, Router, React Query, forms, performance, Error Boundary/Suspense, TypeScript |
| **Research** | Quy trình tìm hiểu, đánh giá nguồn tin |
| **Phân tích** | UI chậm/jank, trade-off hai hướng UI |
| **Teamwork** | Code review, bất đồng kỹ thuật, QA/BA/DevOps |
| **Quy trình** | Git, CI/CD, DoD |
| **Kiến trúc** | Component architecture, technical debt, micro-frontend vs SPA monolith |
| **Bảo mật** | XSS, token storage, CSP/env/secrets |
| **Thực hành** | Live design (product list), review code component có lỗi cố ý |
...
|-------|----------------|
| **Junior 1** | *(Nền tảng)* Nắm JS/React cơ bản (component, JSX, props); làm task UI nhỏ khi có spec và hướng dẫn chi tiết; cần review thường xuyên |
| **Junior 2** | **Junior 1**, cộng thêm: hiểu đúng khái niệm và áp dụng vào component đơn giản; tự hoàn thành task quen thuộc; biết hỏi đúng chỗ khi kẹt; có tư duy học hỏi |
| **Junior 3** | **Junior 2**, cộng thêm: tự code end-to-end trang CRUD (form, list, gọi API), routing, UI theo design; debug lỗi hooks/state thường gặp; đọc hiểu code team; nắm convention và quy trình làm việc |
...
| **Mid 2** | **Mid 1**, cộng thêm: làm việc độc lập trong phạm vi 1 feature với requirement rõ; hiểu trade-off cơ bản (state local vs global, memo); viết code dễ đọc, có test; debug có hệ thống; phối hợp thêm DevOps |
| **Mid 3** | **Mid 2**, cộng thêm: thiết kế giải pháp trong phạm vi module UI; chọn và hướng dẫn Mid 1 áp dụng thư viện/công nghệ; đánh giá technical choice; mentor Junior; xử lý bug/incident production phổ biến |
| **Senior 1** | **Mid 3**, cộng thêm: thiết kế giải pháp feature/system frontend; ra quyết định kỹ thuật có căn cứ (data, POC); hiểu production constraints (perf, a11y, SEO) |
| **Senior 2** | **Senior 1**, cộng thêm: tư duy hệ thống frontend; dẫn dắt kỹ thuật team; thiết lập chuẩn code/review; cân bằng delivery, quality, technical debt |
| **Senior 3** | **Senior 2**, cộng thêm: kiến trúc frontend & scale; technical strategy; dẫn dắt nhiều người; onboarding team; đại diện kỹ thuật với stakeholder |
...
- **Junior:** Ưu tiên nền tảng React; **không** yêu cầu nâng cao / kiến trúc sâu; bảo mật ở mức XSS cơ bản, không lộ token.
- **Mid:** Nền tảng sâu hơn; bật nâng cao + research; kiến trúc component; QA/BA (Mid 1), DevOps (Mid 2).
...
| 11 | [11-live-code-product-list.md](./11-live-code-product-list.md) | Live product list — Mid/Senior |
