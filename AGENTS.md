...
Bạn là agent hỗ trợ xây dựng **Minipower — AI Operating Model của doanh nghiệp** (mô hình vận hành viết thành dạng AI thi hành được) trong repo `minipower` — một hệ **skill + tài liệu + hook** theo module, với cốt lõi: **con người làm gatekeeper ở từng chặng, AI fan-out xử lý song song theo từng module / từng giai đoạn**. Nhiệm vụ: phát triển, mở rộng và bảo trì các module (`sdlc` — pipeline vòng đời, `backend` — skill code .NET, `ops` — kỹ năng vận hành, `toolbox` — công cụ làm ra chính minipower, các module tương lai), hook, template và roles sao cho đúng triết lý, đúng quy ước, và tuân thủ các nguyên tắc code ở phần cuối tài liệu.
...
- **Mục tiêu:** đưa **kỹ năng của từng vị trí** và **quy trình của từng phòng ban — kể cả liên phòng ban** ([contracts/](contracts/README.md)) thành skill AI thi hành được, tích hợp bộ công cụ công ty đang dùng (quản lý việc, wiki, Git, giám sát) để doanh nghiệp vận hành theo quy trình phát triển phần mềm thống nhất — cho cả ba loại dự án `mvp` · `standard` · `maintain`. Lõi hiện hành: `sdlc` dẫn dự án từ *painpoint khách hàng* → *SRS, kiến trúc, kế hoạch, tài liệu bàn giao* qua **6 phase / 19 DOC** chuẩn nghề (IEEE 830, ISO/IEC/IEEE 29148, BABOK, PMBOK, ADR, OpenAPI), giữ mọi thứ **trace được** (UC → FR → AC → Test); `backend` dựng code .NET theo framework Jarvis; `ops` phục vụ DevOps vận hành (metrics, chẩn đoán sự cố).
...
- **"Cứng bằng máy, mềm bằng lời" (QĐ-3).** Chỉ thứ máy kiểm được mà không cần phán đoán mới là điều kiện cứng: **tồn tại file DOC · đường dẫn · ID**. Bảy điều kiện cứng hiện hành — `profile-guard` · `prereq-gate` · `baseline-guard` (baseline + `_legacy`) · `token-guard` · `auto-routing` · `trace:check` (CI) · `link:check` (CI). Mọi thứ khác là **advisory**.
...
- **SKILL.md cho agent, README.md cho người:** SKILL.md = quy tắc/workflow/output bắt buộc; README.md = hướng dẫn, bảng tra, prompt mẫu. Skill mới phải **single-purpose** và **khai trigger để router biết khi nào gọi**: phase-skill map qua `rules.json` (`phase_by_doc`); skill cross-phase (như `deliberation`/`doc-review`/`fan-out`) khai ở **bảng trigger trong router [`sdlc/SKILL.md`](sdlc/SKILL.md)**. Skill **lá-rời trong module** (như `backend/`) không qua router — sống nhờ description: `name` ≡ tên thư mục lá, tiền tố `minipower-{module}-`, ≤64 ký tự, **bắt buộc có `description`** (bề mặt kích hoạt), provider con đặt `name` = `<skill cha>-<provider>` — có test canh (`backend-pack.test.js`, `ops-pack.test.js`, `toolbox-pack.test.js`). Viết skill lá mới: [`minipower-toolbox-skill-author`](toolbox/skills/minipower-toolbox-skill-author/SKILL.md).
...
- **Module (đơn vị cài, chứa skill):** `sdlc/` (Quy trình phát triển — lõi BA+SA+TPM, router-gộp, đăng ký `minipower-sdlc`) · `backend/` (skill implementation .NET, 15 lá-rời `minipower-backend-*-dotnet`) · `ops/` (kỹ năng vận hành DevOps, lá-rời `minipower-ops-*`) · `toolbox/` (công cụ **làm ra chính minipower** — viết/soát skill, mở module; lá-rời `minipower-toolbox-*`, chỉ cài trong workspace repo này, **không** publish sang repo product). **Tầng nền (đọc trực tiếp, không cài):** `contracts/` (hợp đồng liên-pack — schema `PACK.md`) · `ADRs/` (quyết định định hướng). **Kho tạm:** `staging/` (nội dung cũ viết tạm + `interview/`) — **chỉ rút, không nạp**: mỗi file rút dần thành skill/template, tài liệu mới thuộc về một skill chứ không thuộc về đây. Đây là **quy ước, không có cổng máy** — không viết "bắt buộc" cho nó.
- **Trong `sdlc/`:** `skills/{phase}/SKILL.md` (6 phase + `deliberation`/`doc-review`/`readiness-gate`/`fan-out`/`as-built`) · `agents/*.md` (guardrail markdown thuần) · `hooks/{bin,lib,test}/` (Node ESM) · `roles/*.md` (7 lăng kính) · `templates/` (DOC-01–19 + TPL phụ trợ) · `project-skeleton/` + `docs-skeleton/` (khung dự án đích) · `install/{cursor,claude,opencode}/`.
...
- **ADR:** đặt tại `ADRs/`, tên `ADR-NNN-yyyy-MM-dd-slug.md` — **mã `ADR-NNN` bất biến**, cấp theo thứ tự thời gian, không đổi khi trạng thái đổi; **trạng thái KHÔNG nằm trong tên file** mà khai ở `ADRs/README.md` (Pending 🔴 / Todo ⚪ / Doing 🟡 / Done 🟢 / Cancel 🟣). Thêm ADR = tạo file + thêm một dòng vào `ADRs/README.md` trong cùng commit. Mỗi ADR tự khai mục **Ảnh hưởng** (quyết định chi phối nội dung/file nào); file cụ thể **không** trỏ ngược về ADR. Đổi triết lý/phạm vi → ghi/đối chiếu ADR **trước**.
...
### Tham chiếu tài liệu
- `README.md` — bản đồ toàn repo; `sdlc/README.md`, `backend/README.md`, `ops/README.md` — hub từng module.
...
Khi được yêu cầu thêm/sửa feature: xác định đúng skill/hook/template chịu trách nhiệm, kiểm tra tài liệu liên quan trong `ADRs/` + `sdlc/docs/`, tôn trọng triết lý bất biến (mục Bối cảnh) và mô hình gatekeeper + fan-out trước khi viết. Kéo repo về phía "agent tự động hoá" → **dừng và hỏi tôi**.
