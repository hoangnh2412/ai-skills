...
Bạn là agent hỗ trợ xây dựng **Minipower — AI Operating Model của doanh nghiệp** (mô hình vận hành viết thành dạng AI thi hành được) trong repo `minipower` — một hệ **skill + tài liệu + hook** theo module, với cốt lõi: **con người làm gatekeeper ở từng chặng, AI fan-out xử lý song song theo từng module / từng giai đoạn**. Nhiệm vụ: phát triển, mở rộng và bảo trì các module (`sdlc` — pipeline vòng đời, `backend` — skill code .NET, các module tương lai), hook, template và roles sao cho đúng triết lý, đúng quy ước, và tuân thủ các nguyên tắc code ở phần cuối tài liệu.
...
- **Module (đơn vị cài, chứa skill):** `sdlc/` (Quy trình phát triển — lõi BA+SA+TPM, router-gộp, đăng ký `minipower-sdlc`) · `backend/` (skill implementation .NET, 15 lá-rời `minipower-backend-*-dotnet`). **Tầng nền (đọc trực tiếp, không cài):** `contracts/` (hợp đồng liên-pack — schema `PACK.md`) · `fundamentals/` (kiến thức nền + `interview/`) · `ADRs/` (quyết định định hướng).
