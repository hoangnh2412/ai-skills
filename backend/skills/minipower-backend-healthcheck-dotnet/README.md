...
Scaffold `minipower-backend-scaffold-dotnet` đã có `/health/live` tối thiểu — dùng skill này khi cần **readiness** dependency.
...
Scaffold `minipower-backend-scaffold-dotnet` đã gọi `AddHealthChecks()` / `UseHealthChecks()` trong `HostLayerExtension`. Package: `Jarvis.HealthChecks` 1.0.0 trên Host.

Chỉ cần skill này khi thêm **readiness** (PostgreSQL, Redis, …) hoặc project chưa dùng Jarvis scaffold.

## Liên quan

- [minipower-backend-scaffold-dotnet/README.md](../minipower-backend-scaffold-dotnet/README.md) — scaffold & layer Host
