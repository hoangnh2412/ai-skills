...
Ghi rõ gap (ví dụ queue depth nội bộ, cache hit rate) → đề xuất custom `IMetricInstrumentation` qua [minipower-backend-telemetry-dotnet](../minipower-backend-telemetry-dotnet/SKILL.md) — không giả vờ có series trên dashboard.
