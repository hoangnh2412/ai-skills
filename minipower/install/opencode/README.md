...
Plugin gói **một file entry** (`minipower.ts`) + `lib/parts.ts` (glue OpenCode). Logic guard đến từ [hooks/lib/*.js](../../hooks/) dùng chung — `minipower.ts` import qua đường dẫn tương đối `../../../hooks/lib/*.js`, resolve theo **realpath** của pack (nên symlink pack, đừng copy rời file).
...
Ở **message đầu tiên mỗi phiên** (mô phỏng SessionStart), plugin gọi `checkDecisionStaleness` từ [hooks/lib/decision-staleness.js](../../hooks/lib/decision-staleness.js): so ngày DEC (còn hiệu lực) với lịch sử git của DOC trong `Trace:`; DOC đổi sau ngày → chèn cảnh báo vào context. Git thuần qua `child_process`, không cần python. **Không chặn** — lỗi hook không ảnh hưởng luồng. Cùng logic với Cursor/Claude (một file `.js`).
