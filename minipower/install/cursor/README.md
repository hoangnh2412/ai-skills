...
|------|---------|----------------|
| Kiểm tra scope prompt | `check-prompt-scope.ps1` | `check-prompt-scope.sh` |
| Giới hạn đọc baseline/_legacy | `limit-reads.ps1` | `limit-reads.sh` |
...
chmod +x .cursor/hooks/check-prompt-scope.sh .cursor/hooks/limit-reads.sh
```
...
3. Thử `@docs/` hoặc `@docs/03-modules/` không kèm file → bị chặn.
...
```powershell
# Windows
'{"prompt":"@docs/03-modules/"}' | .cursor\hooks\check-prompt-scope.ps1
