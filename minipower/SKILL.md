...
├── memory/                ← Context nhanh (đọc trước khi làm việc)
│   └── memory.md          ← Index memory; link notes/docs theo phase
...
└── docs/                  ← Artifact chính thức Minipower (DOC-01–18)
    ...
    ├── 05-traceability/   ← Ma trận trace, doc registry
...
| [`notes/`](notes/) | Phân tích, trao đổi, decision log; chốt nội dung → **distill** vào `docs/` |
...
| `notes/{phase}/` | File: `YYYY-MM-DD-<stt>-<mo-ta>.md` |
| `docs/03-modules/` | Copy `_template/` → `{module-id}/` khi mở module |
| `memory/memory.md` | Agent đọc **đầu mỗi session** |

**Luồng:** `assets/` → `notes/` → `docs/` → `02-baseline/` · sau baseline → `06-changes/CR-xxx/`

### Thao tác agent khi init

1. Xác nhận `{project}/` path (hoặc tạo folder mới theo tên user cung cấp).
2. Copy [`project-skeleton/`](project-skeleton/) → `{project}/` (README, memory, assets, notes).
3. Copy [`docs-skeleton/`](docs-skeleton/) → `{project}/docs/`.
4. Điền `README.md` và `memory/memory.md`: tên dự án, khách hàng, phase = discovery.
5. **Không** tạo thêm cấu trúc lệch chuẩn trừ khi user yêu cầu rõ.
...
- [ ] Đủ 4 nhánh: `memory/`, `assets/`, `notes/` (6 phase), `docs/` (7 folder)
- [ ] `memory/memory.md` có tên dự án + phase hiện tại
- [ ] Mỗi `notes/{phase}/README.md` tồn tại
