...
description: >-
  ...
  trace, trước baseline) · decision-log (lưu quyết định + phương án bị loại).
---
...
Khi user ghi `Phase: …` hoặc intent rõ (vd. "phân tích yêu cầu", "thiết kế kiến trúc") → **đọc** `memory/{phase}/` (gồm `decision-log.md` nếu có) + skill con tương ứng, rồi áp dụng workflow. Kết thúc phiên → cập nhật `memory/{phase}/` + ghi quyết định có phương án bị loại vào `memory/{phase}/decision-log.md` ([schema](docs/decision-log.md)), **không** append dài vào `memory/memory.md`.
