...
| **CI — chạy thật, xanh 3 OS** | ✅ run #1 `556677d`: ubuntu 7s · windows 22s · macos 9s, **169 pass** | 2026-07-18 |
| **Push** | ✅ `origin/feature/upgrade-brainstorming` (e0545ea..556677d, 4 commit) | 2026-07-18 |
...
## §9. Việc còn lại (tính đến 2026-07-18)

### Đã xong
P0 (gộp Node SSOT + xoá bản cũ) · P2 (test **181** + CI 3 OS xanh) · P3/R2 (3 tầng, Option C) · P4 (doc-sync) · push. 5 bug sửa (BUG-1..5).

### Còn phải làm

| # | Việc | Loại | Ghi chú |
|---|------|------|---------|
| **R1** | **Smoke-test hook trong IDE thật** — Cursor, Claude, OpenCode | ⚠ **verify** | Test+CI chỉ phủ logic+I/O, **không** phủ việc hook nổ trong IDE. OpenCode dễ vỡ nhất (Bun resolve symlink realpath cho `../../../hooks/lib`). Lệnh smoke ở README từng nền tảng |
| **R2** | **P3 — chi phí tương xứng (3 tầng micro/light/full)** | ✅ **xong (Option C)** | Hook `isMicroTask` hạ cảnh báo micro + tiêm gợi ý mềm (deterministic, +12 test → **181 pass**); `SKILL.md` bảng tầng chọn gate light/full (agent, không tốn LLM thêm); pointer ở deliberation + doc-review. Baseline/breadth/discovery/change-control luôn Full |
| **R3** | **P1 — rules.json (rules-as-data)** | 🟡 refactor | §5.2 — trích map DOC→phase + keyword ra 1 file JSON; `agents/auto-routing.md` sinh từ đó. Thêm DOC-19 = 1 dòng. Đã có test làm lưới |
| **R4** | **P4 còn — rubric Complexity 5 chiều × 0–20** | 🟡 nội dung | §3.7 — `skills/planning/SKILL.md` tham chiếu rubric **không tồn tại**. Viết hoặc bỏ |
| **R5** | Thay `/ABSOLUTE/PATH/TO/` (Claude settings) bằng script install + verify | ⚪ polish | §3.6 — còn bắt sửa tay |
| **R6** | Merge branch `feature/upgrade-brainstorming` → `main` (PR) | ⚪ quy trình | Sau khi R1 pass |

### Đề xuất thứ tự
**R1 (smoke-test)** trước — chốt bản Node chạy thật, vì mọi thứ khác xây trên nó. Rồi **R2 (P3)** vì đó là giá trị thiết kế lớn nhất còn lại. R3/R4/R5 làm dần. R6 khi sẵn sàng merge.

### KHÔNG làm (giữ nguyên từ §5.6)
Thêm platform thứ 4 · README "research-backed" · thêm skill mới trước khi R2/R3 xong · bê `plugin.universal.yaml`.

---

**Trạng thái: P0/P2/P4 xong + CI xanh + đã push. Kế tiếp: R1 smoke-test IDE, rồi R2 (P3).**
