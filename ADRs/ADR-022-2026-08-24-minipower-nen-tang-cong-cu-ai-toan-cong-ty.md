...
|---|---|
| **Ngày** | 2026-08-24 |
| **Trạng thái** | 🟡 **Đợt §6 thi hành XONG 8/8 bước (2026-08-25)** cùng [ADR-021](ADR-021-2026-08-24-doi-ten-pack-jarvis-thanh-backend.md) — verify §5 **5/5 PASS** (đối chiếu baseline gốc: 0 gãy ngoài nợ cũ · grep path sạch · 413/413 test + parity · gen:check · nội dung SKILL bất biến). Chuyển 🟢 khi chủ repo xác nhận. **Sau đợt:** `install.mjs --with` (QĐ-6) · mở rộng ánh xạ MCP tên (QĐ-9) · tích hợp OpenProject cho `sdlc` |
