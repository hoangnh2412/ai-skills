...
| 9 | ⚪ | Skill **`as-built`** (người trigger · wrap codegraph MCP\|CLI · degrade Read/Grep · index local + `.gitignore` — Q4) + `docs/06-changes/incident/` + **trigger `as-built` trong router** *(dời từ #5 — chỉ nối được khi skill đã tồn tại)*. *`assets/archive/README.md` đã xong ở #6* | Chạy thử trên 1 repo legacy thật, **có và không có** codegraph |
