...
| 12 | 🟢 | [AGENTS.md](../AGENTS.md): 3 gate **đều mềm** · **phép thử** "cái gì FAIL được bằng máy?" trước khi viết chữ *bắt buộc* · 7 điều kiện cứng hiện hành · `project_mode` · QĐ-14 · bỏ `permissions.deny`, 6 hook · lệnh `trace:check`. [COORDINATION.md](../contracts/README.md): mục **§2.0 handoff là per-module** — `ORD` qua H4 trong khi `INV` chưa qua H2 là **đúng**, không có vạch đích chung. [README](../README.md): bảng use-case 3 chế độ cho người mới | ✅ Test [`docs-consistency.test.js`](../minipower/hooks/test/docs-consistency.test.js) — 11 test canh 3 file gốc repo, thứ trước giờ chỉ dựa vào trí nhớ người sửa |
