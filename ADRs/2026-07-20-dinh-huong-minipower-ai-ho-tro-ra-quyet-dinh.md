...
|---|---|
| **Ngày** | 2026-07-20 |
| **Trạng thái** | 🟡 Định hướng — Q6/Q7 đã chốt (§6), N1–N6 chưa triển khai. Nối tiếp [ADR 2026-07-17](2026-07-17-danh-gia-minipower-va-chien-luoc-phat-trien.md) (P0–P4, R1–R6 đã xong) |
...
| # | Việc | Deliverable | Ưu tiên |
|---|------|-------------|---------|
| **N1** | Readiness Gate (§3.1) | Skill `readiness-gate` **single-purpose**, do auto-routing kích hoạt (không phải người chọn). Cơ chế: (a) liệt kê trọn bộ tiền đề thiếu **một lượt**, (b) mỗi mục cho *trả lời/hoãn/không cần*, (c) sổ nợ open-questions để bổ sung dần. Prereq lưu rules-as-data (`rules.json → prereq_by_intent`). | 🔴 P0 |
| **N2** | Project State Awareness (§3.2) | Thêm `phase_state` vào `rules.json`; auto-routing suy ra giai đoạn dự án → gợi ý bộ skill/lăng kính. Test golden. | 🔴 P0 |
| **N3** | Roles as lenses (§3.3) | `roles/*.md` (BA, PM, SA, DEV, QC, DevOps, Support) — mỗi file: Goal · Deliverables · Checklist · Câu hỏi cần hỏi. Tool-agnostic, gen bảng qua `npm run gen`. | 🟠 P1 |
| **N4** | Context-aware auto-load (§3.4) | Định nghĩa chuỗi nạp ngữ cảnh trong `rules.json`; hook/skill nạp theo intent sau khi N1 pass. | 🟠 P1 |
| **N5** | Templates mở rộng (§3.5) | Thêm template ADR/RFC/Meeting Minutes/Incident/Postmortem vào bộ template. | 🟡 P2 |
| **N6** | Memory linking (§3.5) | Mở rộng schema `decision-log`: trường `affects` (hệ thống/task/release) → truy vết ngược. | 🟡 P2 |
