...
| **ADR-019** | 2026-08-20 | [Harness đa vai, không gate nội bộ](ADR-019-2026-08-20-minipower-harness-khong-gate.md) | ⚪ Todo | **ADR định hướng hiện hành** — pivot: minipower thành **harness** gọi khi cần, **không block bước nào**; phê duyệt sống ở OpenProject/**Gitlab**/Outline/Slack; `readiness-gate` + `doc-review` thành **advisory/báo cáo**; trace giữ, kiểm bằng script `trace:check` **chạy trong CI Gitlab** (fail khi ID trỏ sai/trùng — cưỡng chế ở công cụ ngoài). Huỷ ADR-003/013/017/018. **8 việc triển khai** ở §8 (đụng `rules.json` ⇒ `gen → test → gen:check`). **Chặn:** Q1 luồng duyệt ngoài đã có chưa (rủi ro R4) · Q2–Q5 |
...
*Cập nhật lần cuối: 2026-08-20 · 19 ADR · mã kế tiếp: **ADR-020***
